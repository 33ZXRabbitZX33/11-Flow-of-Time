...
icon_image = pygame.image.load("3.png")
...
birth = datetime.datetime(2006,3,8)
birth2 = birth.timestamp()

count = datetime.datetime(2024,3,9).timestamp() ################################### Start time
global n
n = True
...
def GamePlay(cc):
    ...
    while True:
        ...
        times = datetime.datetime.now().timestamp()
        now5 = datetime.datetime(int("{}".format(now3.year)),3,8).date()
        ye = now5 - now3
